...
__all__ = [
    ...
    "FloatStrategy",
    "StringStrategy",
    "URLPathStringStrategy",
    "HTTPHeaderStringStrategy",
    "XFieldsHeaderStringStrategy",
    "DateStrategy",
    "DateTimeStrategy",
    "UUIDStrategy",
    "FileStrategy",
...
class StringStrategy(PrimitiveStrategy):
    ...
    def strategy(self):
        ...
        if self._blacklist_chars:
            options["alphabet"] = hy_st.characters(
                blacklist_characters=self._blacklist_chars
            )

        if self._min_length:
            options["min_size"] = self._min_length

        if self._max_length:
            options["max_size"] = self._max_length

        strategy = hy_st.text(**options)
...
class ArrayStrategy(PrimitiveStrategy):
    ...
    def strategy(self):
        ...
        return hy_st.lists(
            elements=self._elements.strategy(),
...
class ObjectStrategy(PrimitiveStrategy):
    ...
    def __init__(self, swagger_definition, factory):
        ...
        self._properties = {
            prop_name: self._factory.produce(prop_defn)
            for prop_name, prop_defn in self._swagger_definition.properties.items()
        }

        additional = (
            swagger_definition.additionalProperties
            or len(swagger_definition.properties) == 0
        )
    ...
    def strategy(self):
        ...
        result = base_st.merge_optional_dict_strategy(
        ...
        if self._additional_properties:
            ...
            max_properties = (
                ...
                if self._max_properties is None
                else self._max_properties
            )
            max_properties = min(
                self.MAX_ADDITIONAL_PROPERTIES,
            ...
            log.debug(
                "Determined max, min extra properties: %r, %r",
                max_properties,
                min_properties,
            )
            forbidden_prop_names = set(
                required_properties.keys() & optional_properties.keys()
            )
